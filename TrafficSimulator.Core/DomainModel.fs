...
module DomainModel =    
    ...
    type ConnectionType = Linear | Curved of Curve | QuadraticBezier of QuadraticBezier     
     
    type Connection = {ConnectionType:ConnectionType;StartId:CrossingId;EndId:CrossingId}
      
    type ObjectLocation = {Placing:Connection;CurrentProgress:Fraction}  
    ...
    type Vehicle = {Id:VehicleId;Location:ObjectLocation;CurrentMotionParams:VehicleMotionParams;
    ...
    module ConnectionsGraph =         
        ...
        let calculateCrossingInputs connectionsGraph crossingId = 
            connectionsGraph.CrossingsOutput 
                |> Map.toSeq |> Seq.map (fun (_,coll) -> coll) 
                |> Seq.collect id |> Seq.filter (fun con -> con.StartId = crossingId)
    module DrivePath = 
 ...
 module DomainFunctions = 
    ...
    let bezierFromCrossings crossingsMap crossingStartId crossingEndId controlPolint = 
        ...
        {
            ConnectionType = ConnectionType.QuadraticBezier 
    ...
    module LenghtProviders = 
        ...
        let lenghtProvider (distancePerUnit:float<m>) (connectionsGraph:ConnectionsGraph) connection  =
            match connection.ConnectionType with 
                ...
                |  QuadraticBezier bezier ->let p0 = (connection.Start connectionsGraph).Position 
    ...
    let locationToPosition (connectionsGraph:ConnectionsGraph) (position:ObjectLocation) =
        let startPos = (position.Placing.Start connectionsGraph).Position
        let endPos =   (position.Placing.End connectionsGraph).Position 
        ...
        |  (Position2d pos1,Position2d pos2) -> 
            match position.Placing.ConnectionType with
                ...
                | Linear -> 
                        ...
                        {X = pos1.X + (pos2.X - pos1.X) * currentPos;Y = pos1.Y + (pos2.Y - pos1.Y) * currentPos}
                | QuadraticBezier b ->                                         
                                        let n = (b.segmentsCache.Length - 1) |> float
                                        let temp = ((position.CurrentProgress.Value *  n) |> int)
                                            
                                        let t = b.segmentsCache.[temp]
                                        let t2 = if temp + 1 < b.segmentsCache.Length then b.segmentsCache.[temp + 1] else t
    ...
    module NextConnectionChoosers = 
        ...
        let doNotTurnBack (vehicle:Vehicle) (chooser:NextConnectionChooser) connectionsGraph crossingId =
            ...
            let rec repeat() = 
                match perform with
                    | Some c -> if c.StartId = vehicle.Location.Placing.EndId && (ConnectionsGraph.crossingOutputs connectionsGraph crossingId) |> Seq.length = 1  
                                    then repeat() 
                                else 
                                    Some(c)
                    | None -> None
